...
//接收损坏路产
@property (nonatomic, strong) NSString *luchan;
...
-(NSString*)generateCommonInquireText{
    ...
    NSString* text = [NSString stringWithFormat:@"%@%@%@",@"问：你好! 我们是",organizationName,@"路政员，请问你驾驶的车辆什么车型？什么车牌号码？\n" ];
    NSArray *citizenArray = [Citizen allCitizenNameForCase:self.caseID];
    Citizen *citizen = [citizenArray objectAtIndex:0];
    text = [NSString stringWithFormat:@"%@%@%@%@。\n",text,@"答：我驾驶的是",citizen.automobile_pattern,citizen.automobile_number];
    ...
    text = [NSString stringWithFormat:@"%@问：%@\n%@%@",text,@"以下是路产损坏的具体数据，你是否承认？",@"损坏路产如下：",self.luchan ];
...
-(void)loadCaseInfoArray{
    ...
    if (caseInfo) {
        ...
        [formatter setDateFormat:@"yyyy年M月d日 HH时mm分"];
        ...
        NSString *stationString = [NSString stringWithFormat:@"K%@+%@M",stationStartKMString,stationStartMString];
